    ...
    <div
        ...
        <div class="mx-auto w-full max-w-full p-4">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold text-[#0F3F3B] dark:text-white">EDIT KELAS RS</h1>
            ...
            <div class="mt-6 rounded-2xl bg-[#0F3F3B] p-4 md:p-6 shadow-[0_10px_30px_rgba(0,0,0,0.12)]">
                <div class="rounded-xl bg-[#F5F1E6] p-4 md:p-6">
                    <form method="POST" action="{{ route('persi.pengaturan.member_classes.update', $memberClass->id) }}"
                        class="grid grid-cols-1 gap-4 md:grid-cols-4">
                        @csrf
                        @method('PUT')
                        <div>
                            <label class="mb-1 block text-xs font-semibold text-gray-700">Kode</label>
                            <input type="text" name="code" value="{{ old('code', $memberClass->code) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $memberClass->name) }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="default_dues_amount"
                                value="{{ old('default_dues_amount', (int) $memberClass->default_dues_amount) }}"
