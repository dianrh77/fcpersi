...
@section('content')
    <div
        ...
        <div class="mx-auto w-full max-w-full p-4" x-data="kegiatanRealisasiList">
                ...
                <div class="bg-[#F5F1E6] rounded-xl mx-2 my-2 p-4 md:p-6 dt-orange">
                    ...
                    <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <div class="text-sm font-extrabold tracking-wide text-[#0F3F3B]">
                            PILIH KEGIATAN UNTUK REALISASI
                        </div>
                    ...
                    <div class="overflow-x-auto -mx-2 px-2">
                        <table id="tableKegiatan" class="whitespace-nowrap w-full"></table>
                    </div>
                </div>
...
@section('scripts')
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('kegiatanRealisasiList', () => ({
                all: @json($kegiatans ?? []),
                table: null,
                ...
                init() {
                    ...
                    const headings = ['Aksi', 'No Kegiatan', 'Tanggal', 'Kegiatan', 'Pos', 'Anggaran',
                        'Status'
                    ];
                    const rows = (this.all || []).map(k => ([
                        String(k.id ?? '-'),
                        String(k.activity_no ?? '-'),
                        String(k.activity_date ?? '-'),
                        String(k.activity_name ?? '-'),
                        String(k.pos ?? '-'),
                        String(k.budget_amount ?? 0),
                        String(k.status ?? '-'),
                    ]));

                    this.table = new simpleDatatables.DataTable('#tableKegiatan', {
                        searchable: true,
                        perPage: 25,
                        ...
                        columns: [{
                            ...
                            },
                            {
                                ...
                                render: (cell) => {
                                    ...
                                    const base = @json(url('/persi/kegiatan/realisasi'));
                    ...
                    this.table.columns().sort(2, 'desc');
                }
            }))
        })
    </script>
